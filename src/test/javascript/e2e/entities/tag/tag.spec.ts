...
describe('Tag e2e test', () => {
  ...
  it('should load Tags', async () => {
    ...
    expect(await tagComponentsPage.getTitle()).to.eq('plannerApp.tag.home.title');
  });

  it('should load create Tag page', async () => {
    await tagComponentsPage.clickOnCreateButton();
    tagUpdatePage = new TagUpdatePage();
    expect(await tagUpdatePage.getPageTitle()).to.eq('plannerApp.tag.home.createOrEditLabel');
  ...
  /* it('should create and save Tags', async () => {
    ...
    expect(await tagUpdatePage.getIconInput()).to.eq('icon', 'Expected Icon value to be equals to icon');
